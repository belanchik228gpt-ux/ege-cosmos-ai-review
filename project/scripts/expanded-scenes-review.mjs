...
const profile = path.resolve(`test-results/expanded-scenes-${Date.now()}`),
...
try {
  ...
  const { historyAtlasDrawing } = await server.ssrLoadModule(
    '/src/domain/topic-scenes/history-atlas.ts',
  );
  const { civicsExpandedDrawing } = await server.ssrLoadModule(
    '/src/domain/topic-scenes/civics-expanded.ts',
  );
  const { chemistryExpandedDrawing } = await server.ssrLoadModule(
  ...
  for (const unit of schoolUnits)
    for (const focus of [...unit.topics, unit.title]) {
      const d =
        historyAtlasDrawing(unit, focus) ||
        civicsExpandedDrawing(unit, focus) ||
        chemistryExpandedDrawing(unit, focus);
...
try {
  ...
  for (const [i, c] of cases.entries()) {
    ...
    if (c.figure) {
      const firstText = scene.locator('[data-subject-figure] text[font-size]').first();
      const actual = await firstText.evaluate(el=>({size:getComputedStyle(el).fontSize, expected:el.getAttribute('font-size')}));
      assert.equal(actual.size,actual.expected+'px','Application CSS overrides authored SVG text');
    }
    assert(
  ...
  for (const figure of ['history-map', 'chemistry-reaction']) {
  ...
  const small = cases.find((c) => c.figure === 'chemistry-reaction');
  const scene = await open(small);
  await scene.getByRole('button', { name: 'Кадр 3', exact: true }).click();
  await shot(page, 'chemistry-1280x720');
  ...
  await shot(scene, 'chemistry-reduced-motion');
